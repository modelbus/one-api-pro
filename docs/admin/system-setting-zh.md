...
| `PasswordLoginEnabled` | 关闭后用户只能走 OAuth / WeChat / Lark 登录 |
...
| `SystemName` / `Logo` / `Theme` | 站点名称 / Logo URL / 前端主题名（必须 ∈ `config.ValidThemes`） |

## 联动校验 / Toggle Preconditions

`controller/option.go::UpdateOption` 在启用部分开关前会校验前置字段（详见 [operation-setting](operation-setting)）。

## OAuth 登录接入流程 / OAuth Setup
