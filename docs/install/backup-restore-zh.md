...
| 类型| 路径| 备注|
| --- | --- | --- |
| 主数据库| `SQLITE_PATH`（默认 `one-api-pro.db`）或外部 MySQL/PostgreSQL | 包含全部业务表与系统设置 |
| 日志数据库| `LOG_SQL_DSN`（若设置） | 高频写入，独立备份即可 |
| 上传 / 上传缓存| `/app/data` 下的运行时缓存 | 临时文件可重建 |
| 日志| `--log-dir`（默认 `./logs`） | 用于排障与审计 |
| 配置| `/app/config/.env`、数据库 `options` 表 | 与备份联动 |
...
### 简单复制（停机或低写入）/ Simple copy (downtime
