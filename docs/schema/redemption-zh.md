...
| 项 | 在哪里 | 设置后影响 |
|---|---|---|
| 兑换码类型 | 兑换码管理 → 新增 | 「额度」直接加 User.quota；「套餐」创建 Subscription |
| 额度数量 | 兑换码管理 → 新增 | 兑换后写入额度 |
| 关联套餐 | 兑换码管理 → 新增 | 仅「套餐」类型需要；兑换后激活 |
| 兑换码有效期 | 兑换码管理 → 新增 | 过期后该码不可再兑换 |
| 单码可兑人数 | 兑换码管理 → 新增 | 默认 1 |

## 关键字段（每张码）

| 字段 | 含义 | 设置后影响 |
|---|---|---|
| 兑换码 | 字符串 | 用户输入的凭证；改了就发不出去了 |
| 类型 | 额度 / 套餐 | 决定兑换后的入账动作 |
| 额度值 | 数字 | 兑换后写入额度（仅「额度」类型） |
| 关联套餐 | Plan FK | 兑换后激活的套餐（仅「套餐」类型） |
| 总次数 | 默认 1 | 大于 1 时支持「一码多兑」 |
| 已兑次数 | 数字 | 自动递增；满后失效 |
| 过期时间 | Unix 秒 / 0 = 永不过期 | 过期后失效 |
| 状态 | 未使用 / 已用完 / 已过期 | 决定是否还能兑换 |
