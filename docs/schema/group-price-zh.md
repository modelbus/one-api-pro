...
| 字段 | 含义 | 设置后影响 |
|---|---|---|
| 用户组 | `default` / `vip` / `svip` 等 | 与用户的 group 字段匹配才生效 |
| 模型 | `gpt-4o` / `claude-sonnet-4` 等 | 空字符串表示该用户组**所有模型**的默认折扣 |
| 折扣倍率 | 数字，1.0 = 原价 | 0.8 = 八折；1.2 = 加价 20% |
...
- 不要给同一对 (group, model) 设置多条记录（重复会报唯一键错误）
