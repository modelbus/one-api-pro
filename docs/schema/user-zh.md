...
| 字段 | 含义 | 设置后影响 |
|---|---|---|
| 用户名 | 登录账号 | 修改后下次登录用新用户名 |
| 邮箱 | 找回密码 / 通知 | 改邮箱需要重新走验证 |
| 密码 | 登录凭证 | 重置后用户被踢下线 |
| 用户组 | `default` / `vip` / `svip` 等 | 影响 [分组折扣](/schema/group-price) 命中与可访问模型范围 |
| 状态 | 启用 / 禁用 | 禁用后无法调用任何 API、无法登录 |
| 额度 `quota` | 余额（按内部单位计） | 扣调用费时优先扣这里；扣到 0 直接拒绝 |
| 已用额度 `used_quota` | 累计已扣 | 仅记录，不影响行为 |
| 角色 `role` | 普通用户 / 管理员 / Root | Root 才能进后台；普通用户只能看到自己 |
| 邀请码 / 被邀请人 | 邀请关系 | 注册时填写邀请码，双方获奖励额度 |
