...
| `name` | 备注用 | 不影响行为 |
| `user_id` | 关联到 [User](/schema/user) | 删除用户会级联删除 Token |
| `status` | 启用 / 禁用 / 过期 / 额度耗尽 | 禁用后用此 Key 调用 API 直接被拒绝 |
| `remain_quota` | Token 独立额度 | 扣到 0 即拒绝；不影响用户 `quota` |
| `quota` | 本次发放的额度 | 与 `remain_quota` 不同，仅记录初始额度 |
| `expired_time` | Unix 秒 / `-1` 表示永不过期 | 过期后状态变为「已过期」 |
| `models` | 空数组表示不限制 | 只允许调用列出的模型；非空时其他模型直接 403 |
| `subnet` | CIDR 列表 | 客户端 IP 不在白名单则 403 |
