...
| 字段 | 含义 | 设置后影响 |
|---|---|---|
| `node_id` | 节点 ID | 全集群唯一标识；修改后该节点被认为「换了身份」 |
| `node_name` | 节点名称 | 展示用；修改不影响通信 |
| `status` | 状态 | 启用 / 禁用；禁用后不再接收其他节点的推送 |
| `address` | `host:port` | 节点间 HTTP 推送目标；写错则推送失败 |
| `secret` | 密钥 | 节点间互信的 Bearer Token；必须与集群内其他节点一致 |
| `last_heartbeat_time` | 最后心跳时间 | Unix 秒；长时间未更新 → 标红「失联」 |
