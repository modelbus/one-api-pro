      ...
      <Table basic={'very'} compact size='small'>
        <Table.Header>
          <Table.Row>
            ...
            <Table.HeaderCell>{t('user.table.plans')}</Table.HeaderCell>
            <Table.HeaderCell
              style={{ cursor: 'pointer' }}
              onClick={() => {
                sortUser('role');
            ...
            .map((user, idx) => {
              ...
              return (
                <Table.Row key={user.id}>
                  ...
                  <Table.Cell>
                    {user.plans && user.plans.length > 0 ? user.plans.map((plan, i) => (
                      <Label key={i} basic color='teal' size='tiny' style={{ marginBottom: 2 }}>
                        {plan.plan_name} · {plan.billing_type === 'token' ? 'Token' : plan.billing_type === 'request' ? t('subscription.billing_type.request') : plan.billing_type} · {timestamp2string(plan.end_time)}
                      </Label>
