...
const LogsTable = () => {
  ...
  return (
    <>
      <Header as='h3'>
        ...
        {showStat && <>
          ...
          <span style={{ marginLeft: 8, fontSize: '0.85em', color: '#888' }}>
            （{t('log.normal_quota')}：{renderQuota(stat.normal_quota, t)} | {t('log.subscription_quota')}：{renderQuota(stat.subscription_quota, t)}）
          </span>
        </>}
        {!showStat && (
          <span
      ...
      <Table basic={'very'} compact size='small' style={{ minWidth: '1200px' }}>
        <Table.Header>
          <Table.Row>
            <Table.HeaderCell
              style={{ cursor: 'pointer' }}
            ...
            .map((log, idx) => {
              ...
              return (
                <Table.Row key={log.id}>
                  ...
                  <Table.Cell>
                    {log.billing_source === 1 ? (
                      <Label basic color='blue' size='tiny'>{t('log.billing_source.subscription')}</Label>
                    ) : (
                      <Label basic size='tiny'>{t('log.billing_source.quota')}</Label>
                    )}
                  </Table.Cell>
                  <Table.Cell>
                    {log.plan_name ? (
                      <Label basic color='teal' size='tiny'>{log.plan_name}</Label>
                    ) : (
                      '-'
                    )}
                  </Table.Cell>
