...
**基于 Go 语言的企业级 AI API Gateway —— 在 [one-api](https://github.com/songquanpeng/one-api)（作者 [JustSong](https://github.com/songquanpeng)）基础上深度重构与扩展，配备全新的 Vue 3 管理后台与去中心化多活集群。**
...
**One Api Pro** 是一个基于 Go 语言的**企业级 AI API 网关**，在上游 [one-api](https://github.com/songquanpeng/one-api)（原作者 [JustSong](https://github.com/songquanpeng)）基础上深度重构而来。由于 one-api 已于 2024 年停止更新，One Api Pro 在延续其能力的同时，进行了架构级重写，并配套全新的 Vue 3 管理后台，提供面向团队规模与跨地域部署的生产级能力。

它将 **30+ 家大模型平台**（OpenAI / Anthropic / Gemini / DeepSeek / 通义千问 / 文心一言 / 讯飞 / 智谱 等）统一在 OpenAI 兼容接口之后，所有已支持 OpenAI 协议的客户端工具都可以零成本接入。

One Api Pro 的差异化能力：

- 🧱 **Adaptor 自注册机制** —— 新增供应商只需创建一个包并注册，框架代码零修改。
- 🔐 **精细化多级权限** —— Guest / User / Admin / Root 四级，修复了上游 API 鉴权漏洞。
- 📦 **套餐订阅体系** —— 按 Token 或按次计费，周期限频，按模型精细管控。
- 🌐 **去中心化多活集群** —— 各节点独立 MySQL + Redis，应用层事件驱动同步，无需共享数据库，天然适合全球就近接入。
- 🖥️ **Vue 3 + Arco Design 管理后台** —— 可视化仪表盘、令牌粒度管控、子网限制、审计日志等。
...
- [ ] **支付宝 / 微信官方对接**：原生接入支付宝当面付、微信支付 Native / H5，在线充值自动到账
- [ ] **订阅套餐 Token Plan**：更灵活的订阅套餐 Token 方案，支持按周期自动续费与用量升级
...
- [one-api](https://github.com/songquanpeng/one-api) — 上游原项目（作者 [JustSong](https://github.com/songquanpeng)）
