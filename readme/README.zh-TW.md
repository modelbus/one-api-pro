...
- [x] **統一訂單管理（管理員視角）**：管理端訂單中心支援列表／搜尋／詳情／修改／刪除，依狀態、類型、來源、使用者、套餐多維篩選，可人工標記「已支付」（線下／銀行立即啟用）與「已退款」（僅狀態標記）
- [x] **通道診斷與智慧路由（基礎能力）**：自動冷卻（`CooldownFilter`）、託底降級（`FallbackFilter`）與低成功率自動停用（`monitor`）已上線
- [x] **多語言國際化（i18n）**：管理後台、落地頁與法律頁全量中英文，選單／頁面文案／Arco Design 元件隨語言即時切換，語言選擇持久化並同步 `<html lang>`；語言包按頁面模組拆分並自動深合併

### 🔄 進行中

- [ ] **通道診斷與智慧路由（增強）**：在已上線的自動冷卻／託底降級／低成功率自動停用基礎上，補全獨立診斷面板／節點級 ping 與人工複核流程
- [ ] 更豐富的用量分析報表與匯出
- [ ] **加值退款閉環**：目前訂單 `status=3` 僅標記狀態，quota 不退回；待退款功能開發後補齊

### 🔭 規劃中

- [ ] **更多語言支援**：在中文／英文基礎上擴展繁體中文、日語、韓語、俄語、德語、阿拉伯語等（與 README 多語言版本對齊）
- [ ] **支付通道擴充**：Apple Pay、銀聯、Stripe 等
- [ ] **訂單退款能力**：異步退款 API + 自動化退款流水，套餐／加值退款時訂閱、quota 回退與視覺化退款流水
