...
<p align="center">
  <strong>OneAPI</strong> Pro · Go로 구축된 엔터프라이즈급 AI API Gateway
</p>
<p align="center">
  본 프로젝트는 <a href="https://github.com/songquanpeng/one-api">one-api</a>을 기반으로 심층 재구축했으며, 원저자의 오픈소스 기여에 감사를 표합니다.
</p>
...
- [x] **통합 주문 관리(관리자 관점)**: 관리자 주문 센터가 목록 / 검색 / 상세 / 수정 / 삭제를 지원하고 상태·유형·출처·사용자·플랜별 다차원 필터 제공; 관리자는 「결제 완료」(오프라인 / 은행은 즉시 활성화)와 「환불 완료」(상태 표시만)를 수동으로 표시 가능
- [x] **채널 진단 및 지능형 라우팅(기본 기능)**: 자동 쿨다운(`CooldownFilter`), 폴백 다운그레이드(`FallbackFilter`), 저성공률 자동 비활성화(`monitor`)가 운영 중
- [x] **다국어 국제화(i18n)**: 관리 콘솔·랜딩 페이지·법률 페이지 전면 중/영 지원, 메뉴·페이지 문구·Arco Design 컴포넌트가 실시간 전환, 언어 선택은 저장되고 `<html lang>`와 동기화; 언어 팩은 페이지 모듈 단위로 분리되어 자동 심층 병합

### 🔄 진행 중

- [ ] **채널 진단 및 지능형 라우팅(강화)**: 운영 중인 자동 쿨다운 / 폴백 / 저성공률 자동 비활성화를 기반으로 독립 진단 패널 / 노드 수준 ping / 사람 검토 절차 완성
- [ ] 더 풍부한 사용량 분석 리포트 및 내보내기
- [ ] **환불 폐루프**: 현재 `status=3` 주문은 상태만 변경되고 quota는 복구되지 않음; 환불 기능 출시 후 보완 예정

### 🔭 계획 중

- [ ] **더 많은 언어 지원**: 중국어/영어 외에 번체 중국어, 일본어, 한국어, 러시아어, 독일어, 아랍어 등 확장 (README 다국어 버전과 정렬)
- [ ] **결제 채널 확장**: Apple Pay, UnionPay, Stripe 등
- [ ] **주문 환불 기능**: 비동기 환불 API + 자동화 환불 흐름, 플랜 / 충전 환불 시 구독·quota 롤백 및 시각적 환불 흐름
