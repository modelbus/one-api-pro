...
<p align="center">
  <strong>OneAPI</strong> Pro · 基于Go语言的企业级 AI API Gateway
</p>
<p align="center">
  本项目基于 <a href="https://github.com/songquanpeng/one-api">one-api</a> 深度重构开发，感谢原作者的开源贡献。
</p>
...
- [x] **统一订单管理（管理员视角）**：管理端订单中心支持列表 / 搜索 / 详情 / 修改 / 删除，按状态、类型、来源、用户、套餐多维过滤，可人工标记「已支付」（线下 / 银行立即激活）与「已退款」（仅状态标记）
- [x] **渠道诊断与智能路由（基础能力）**：自动冷却（`CooldownFilter`）、托底降级（`FallbackFilter`）与低成功率自动禁用（`monitor`）已上线
- [x] **多语言国际化（i18n）**：管理后台、落地页与法律页全量中英文，菜单 / 页面文案 / Arco Design 组件随语言实时切换，语言选择持久化并同步 `<html lang>`；语言包按页面模块拆分，支持自动深合并

### 🔄 进行中

- [ ] **渠道诊断与智能路由（增强）**：在已上线的自动冷却 / 托底降级 / 低成功率自动禁用基础上，补全独立诊断面板 / 节点级 ping 与人工复核流程
- [ ] 更丰富的用量分析报表与导出
- [ ] **充值退款闭环**：当前订单 `status=3` 仅标记状态，quota 不回退；待退款功能开发后补全

### 🔭 规划中

- [ ] **更多语言支持**：在中文 / 英文基础上扩展繁体中文、日语、韩语、俄语、德语、阿拉伯语等（与 README 多语言版本对齐）
- [ ] **支付通道扩展**：Apple Pay、银联、Stripe 等
- [ ] **订单退款能力**：异步退款 API + 自动化退款流水，套餐 / 充值退款时订阅、quota 回退与可视化退款流水
