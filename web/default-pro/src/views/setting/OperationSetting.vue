<template>
  <div class="operation-page">
    <a-spin :loading="loading">
          ...
          <div class="section">
            <h3>支付</h3>
            <a-tabs v-model:active-key="paymentTab">
              <a-tab-pane key="wechat" title="微信支付">
                <a-form layout="vertical" class="setting-form">
                  <a-form-item label="启用微信支付">
                    <a-switch v-model="paymentSettings.wechat.enabled" />
                  </a-form-item>
                  <a-row :gutter="[16, 8]">
                  ...
                  <a-form-item>
                    <a-upload :custom-request="(opt) => uploadPaymentFile('wechat', opt)" :show-upload-list="false" accept=".pem">
              ...
              <a-tab-pane key="alipay" title="支付宝">
                <a-form layout="vertical" class="setting-form">
                  <a-form-item label="启用支付宝">
                    <a-switch v-model="paymentSettings.alipay.enabled" />
                  </a-form-item>
                  <a-row :gutter="[16, 8]">
              ...
              <a-tab-pane key="bank" title="银行转账">
                <a-form layout="vertical" class="setting-form">
                  <a-form-item label="启用银行转账">
                    <a-switch v-model="paymentSettings.bank.enabled" />
                  </a-form-item>
                  <a-row :gutter="[16, 8]">
...
async function loadPlanSettings() {
  try {
    const { data } = await settingApi.getPlan()
    if (data.success && data.data) {
      planSettings.upgrade_mode = data.data.upgrade_mode || 'price_diff'
      planSettings.allow_topup = !!data.data.allow_topup
    }
  } catch (e) {}
}
async function savePlanSettings() {
  try {
    const { data } = await settingApi.putPlan({ upgrade_mode: planSettings.upgrade_mode, allow_topup: planSettings.allow_topup })
    if (data.success) Message.success('已保存')
...
async function loadPaymentSettings() {
  try {
    const { data } = await settingApi.getPayment()
    if (data.success && data.data) {
      for (const method of ['wechat', 'alipay', 'bank']) {
        const m = data.data[method + '_enabled']
        const c = data.data[method + '_config']
