  ...
  <div class="chat-page">
    <a-spin :loading="loading">
      <div v-if="loadError" class="chat-error">
        <a-result status="error" :title="$t('chatPage.loadFailed')" :subtitle="loadError">
          <template #extra>
            <a-button @click="fetchChatLink">{{ $t('chatPage.retry') }}</a-button>
          </template>
        </a-result>
      </div>
      ...
      <div v-else-if="loaded" class="chat-empty">
        <a-empty :description="$t('chatPage.emptyDesc')" />
      </div>
    </a-spin>
  </div>
</template>
...
async function fetchChatLink() {
  ...
  try {
    ...
    loadError.value = e.response?.data?.message || e.message || t('chatPage.loadFailed')
  } finally {
    loading.value = false
  }
}
